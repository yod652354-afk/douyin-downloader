...
window.addEventListener('message', (e) => {
  ...
  if (e.data.type === 'FETCH_SHARE_URL') {
    ...
    window.fetch(base + '?' + params.toString(), { credentials: 'include' })
      ...
      .then(d => {
        const result = d?.data || d?.short_url || '';
        console.log('[抖音下载器] 分享链接结果: ' + (result || '空'));
        window.postMessage({ source: '__douyinDL_page', type: 'SHARE_URL_RESPONSE', requestId, ok: true, shortUrl: result }, '*');
      })
      .catch(e => {
